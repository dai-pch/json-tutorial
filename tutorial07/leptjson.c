    ...
    switch (v->type) {
        ...
        case LEPT_ARRAY:
			...
			if (v->u.a.size != 0) {
				...
				for (size_t ii = 1;ii < v->u.a.size;++ii) {
					PUTC(c, ',');
					lept_stringify_value(c, v->u.a.e + ii);
				}
			}
			PUTC(c, ']');
            break;
        case LEPT_OBJECT:
			...
			if (v->u.o.size != 0) {
				...
				for (size_t ii = 1;ii < v->u.o.size;++ii) {
					PUTC(c, ',');
					lept_stringify_string(c, (v->u.o.m + ii)->k, (v->u.o.m + ii)->klen);
					PUTC(c, ':');
					lept_stringify_value(c, &(v->u.o.m + ii)->v);
				}
			}
			PUTC(c, '}');
            break;
